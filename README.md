...
[![Tests](https://img.shields.io/badge/tests-514%20passing-brightgreen)](tests/)
...
**宽进严出**：模型输出的*形状*被宽容对待，*结构*被严格校验（Agent 白名单、工具表、权限）。计划可以写成 `tasks` /
...
```bash
python -m unittest discover -s tests    # 514 个用例，无需 pytest 与网络
```
